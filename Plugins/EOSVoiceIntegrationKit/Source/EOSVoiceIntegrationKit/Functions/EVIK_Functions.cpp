...
void UEVIK_Functions::EOSRoomToken(APlayerController* PlayerController, FString VoiceRoomName, FString PlayerName, FString ClientIP, const FEIKRoomTokenResultDelegate& Result)
{
    ...
    bSuccess &= GConfig->GetString(TEXT("EOSVoiceChat"), TEXT("EncryptionKey"), EncryptionKey, GEngineIni);

    if (!bSuccess)
    {
        UE_LOG(LogTemp, Error, TEXT("Missing values in DefaultEngine.ini"));
    	UE_LOG(LogTemp, Warning, TEXT("Code 2"));
    	Result.ExecuteIfBound(false, "Error");
        return;
    }
...
void UEVIK_Functions::JoinEOSRoom(APlayerController* PlayerController, FString VoiceRoomName, FString ChannelCredentialsJson, const FEIKResultDelegate& Result)
{
    if (PlayerController)
    {
        if (const UGameInstance* GameInstance = PlayerController->GetGameInstance())
            ...
            if (LocalVoiceSubsystem)
            {
                if (LocalVoiceSubsystem->EVIK_Local_GetVoiceChat())
                {
                    ...
                    if (FJsonSerializer::Deserialize(JsonReader, ChannelCredentialsObject) && ChannelCredentialsObject.IsValid())
                    {
                        ...
                        LocalVoiceSubsystem->EVIK_Local_GetVoiceChat()->JoinChannel(VoiceRoomName, ChannelCredentialsJson, EVoiceChatChannelType::NonPositional, FOnVoiceChatChannelJoinCompleteDelegate::CreateLambda([Result](const FString& ChannelName, const FVoiceChatResult& JoinResult)
                        {
